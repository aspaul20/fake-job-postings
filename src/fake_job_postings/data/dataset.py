...
class JobDataset(Dataset):
    def __init__(self, df, meta, tokenizer, logger=None):
        ...
        self.cat_data = torch.tensor(self.df[self.meta['cat_cols']].values, dtype=torch.long)
        self.text_data = self.df['text'].values
        self.labels = torch.tensor(self.df[self.meta['target']].values, dtype=torch.long)
...
class JobDataLoader(pl.LightningDataModule):
    ...
    def setup(self, stage=None):
        ...
        self.train_dataset = JobDataset(train_df, meta=self.meta, tokenizer=self.tokenizer, logger=self.logger)
        self.val_dataset = JobDataset(val_df, meta=self.meta, tokenizer=self.tokenizer, logger=self.logger)
